...
import { FiPlay, FiPause, FiSkipBack, FiSkipForward, FiPlus } from "react-icons/fi";
...
// … müzik geldiğinde, audio.src atandığı useEffect’in altına hemen:
  useEffect(() => {
    // metadata yüklendiğinde toplam süreyi al
    const onLoaded = () => setDuration(audio.duration)
    // oynatma ilerledikçe currentTime güncelle
  ...
  // ─── NEW: fetch emotion-driven music once a real chapter is ready ───────────
  useEffect(() => {
    if (!checkingAnalysis && currentIndex > 0) {
      console.log("▶️ Fetching music for", id, "chapter", currentIndex);
      fetch(`http://localhost:5000/api/music/${id}/${currentIndex}`)
        .then(r => {
          console.log("↩ Received status", r.status);
          return r.json();
        })
        .then(data => {
          console.log("🎵 musicRecs payload:", data);
          setMusicRecs(data);
        })
        .catch(err => {
      ...
      console.log("⏳ Not fetching music yet:", { checkingAnalysis, currentIndex });
    }
  }, [id, currentIndex, checkingAnalysis]);

  // ──────────────────────────────────────────────────────────────────────────
    // whenever the chosen track changes, swap out the audio.src
   // 1) TrackIndex veya musicRecs değişirse yeni kaynağı ata
  useEffect(() => {
    if (musicRecs.length === 0) return;
    audio.src = musicRecs[trackIndex].url;
    // eğer halihazırda oynatılıyorsa, hemen başlat
    if (isPlaying) {
      audio
        .play()
        .catch(() => {
          /* bazen autoplay kısıtlaması olabilir, sessizce yoksay */ 
        });
    }
  }, [trackIndex, musicRecs, audio, isPlaying]);

  // 2) play/pause state’i değişince tetiklenir
  useEffect(() => {    
    if (!audio.src) return;
  ...
  const handlePlayPause = () => {
    // eğer henüz src atanmamışsa, atayıp sonra toggle et
    if (!audio.src && musicRecs.length > 0) {
      audio.src = musicRecs[trackIndex].url;
    }
  ...
  const handleAddToPlaylist = () => {
    const rec = musicRecs[trackIndex];
    // playlist’i localStorage’dan oku, yoksa boş dizi
    const stored = JSON.parse(localStorage.getItem("myPlaylist")) || [];
    // Aynı dosyayı/mood’u tekrar eklememek için kontrol
    if (!stored.find(item => item.file === rec.file && item.mood === rec.mood)) {
      stored.push({
        file: rec.file,
        title: rec.title,
        url: rec.url,
      ...
      localStorage.setItem("myPlaylist", JSON.stringify(stored));
    }
    setIsAdded(true);
        ...
        {musicRecs.length > 0 ? (
          <>
            ...
            <div className="d-flex align-items-center gap-2">
              {/* optional cover */}
              <img
                src={`https://picsum.photos/seed/${trackIndex}/40/40`}
                alt="cover"
                style={{
                  width: "40px",
                  height: "40px",
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  setTrackIndex((i) =>
                    Math.min(musicRecs.length - 1, i + 1)
                  )
                }
              >
             ...
             {/* Add to Playlist */}
              ...
              {/* Eklendi bildirimi */}
              {isAdded && (
                <div style={{ color: "#0f0", marginLeft: "0.5rem" }}>
                  ✓ Added to “{musicRecs[trackIndex].mood}” playlist
                </div>
              )}
